...
**她是怎麼進到那份名單的，2026-09-20 才查清楚**：她在 Gmail 裡按了「取消訂閱」。
那一顆按鈕是說給寄信商聽的，而我們的信裡沒有自己的出口——完整的因果與修法見
下一節〈信裡要有我們自己的出口〉。**這一節擋的是「進了黑名單之後的痛」，
那一節擋的是「為什麼會進去」，兩者互補。**
...
**2026-09-20 查清楚了 helen 那件事的根因，而它比原本以為的更根本：**

> 她在 **Gmail 裡按了「取消訂閱」**，寄信商收到之後把她加進帳號層級的退訂名單，
> 於是連她自己按「忘記密碼」索取的信一起擋掉。

沒有人做錯任何一步。她要的只是「這種信別再來」，拿到的卻是「你再也收不到任何
東西」。而**她會去按那顆按鈕，是因為我們的信裡沒有自己的出口**——唯一的開關在
「我的帳號」頁裡，信的結尾甚至寫著「可在系統內設定」卻沒給任何一條路過去。
那正是〈「我的帳號」分頁〉那句「沒有人找得到的功能等於不存在」，只是這一次的
代價是別人收不到密碼重設信。

**所以這件事有兩個各自獨立的洞，而它們回答的是不同的問題：**

| | 回答什麼 | 狀態 |
|---|---|---|
| 信裡沒有自己的退訂連結 | **為什麼會進黑名單** | 這一節修掉 |
| 交易信與訂閱信共用一個帳號 | **進了為什麼那麼痛** | 見上一節，還沒分家 |

兩者互補，不是二選一。沒有分家的情況下，這個連結就是**唯一**擋得住同一件事
再發生的東西。

| 決定 | 理由 |
|---|---|
| **三層都要有**，不是三選一 | ①`List-Unsubscribe` 標頭 ②信裡看得見的按鈕 ③系統裡的取消訂閱區。第一層擋的是**按信箱軟體那顆按鈕的人**（最多人、也是 helen 那一次的路徑），第二層擋「在信裡找出口的人」，第三層給「想自己管全部的人」。少任何一層，那一群人就會回到 Gmail 那顆按鈕上 |
| **`List-Unsubscribe` 是治本的那一層** | 有了它，Gmail／Apple Mail 的「取消訂閱」會用 POST 打到**我們的端點**，而不是去跟寄信商告狀。同一個動作，結果從「整個帳號被封鎖」變成「只關掉這一種信」。**一開始誤以為做不到**——當時是照 MCP 工具的簡化 schema 推的，而官方 OpenAPI 的 `SendMessageRequest` 有 `headers` 欄位（字串 map）。**規格要看原始的那一份** |
| 兩個標頭要**成對出現** | 少了 `List-Unsubscribe-Post: List-Unsubscribe=One-Click`，收信端會退回舊行為：把網址當成「開給人點的連結」而不是「可以直接 POST 的端點」，於是一鍵退訂不生效，掃描器的 GET 又變成風險 |
| 一鍵退訂**只認 POST**，而且**不回頁面** | 對面是機器不是人。RFC 8058 用 POST 正是為了讓掃描器的 GET 不會誤退訂 |
| 兩個入口**共用同一支 `applyUnsubscribe`** | 複製第二份的症狀是「從信箱軟體退訂沒有用、從我們的頁面退訂有用」——兩邊都各自看起來正常，而沒有人會發現。同〈`TX_KINDS` 只寫一份〉 |
| **交易信不帶這個標頭** | 密碼重設信沒有「退訂」這個概念，帶了反而會讓郵件軟體把它當成廣告 |
| **純文字版也要有那一行** | 有些人就是在純文字模式讀信。只有 HTML 版有出口，等於對那些人來說沒有出口 |
| 按鈕要**明顯**，這是刻意改過的決定 | 第一版是 12px 的灰色文字連結藏在最下面——那是在「不要鼓勵退訂」與「要讓人找得到出口」之間**選錯了邊**。找不到出口的代價不是少一個訂閱者，是那個人去按信箱軟體的「取消訂閱」，然後整個帳號被封鎖、連密碼重設信都收不到。多幾個人退訂遠比那個便宜 |
| 信裡給**兩條路**，分工不同 | 「取消訂閱」（不用登入，一下就停掉這一種）與「通知設定 →」（`#notify`，回系統管全部五個開關）。想少收一種信的人不該被逼著去登入 |
| `#notify` 要**真的走得到底** | 切到帳號頁、捲過去、而且**標示出來**。只切頁籤不捲過去，使用者看到的是一整頁設定還要自己找——那與沒有這條連結差不多，而「差不多」的代價是他回去按 Gmail 那顆。也要接 `hashchange`：已經開著 app 的人再點一次連結，瀏覽器只會改 hash 不會重新載入 |
| 「全部都不要了」**只關不開** | 一顆會在「全關／全開」之間切換的按鈕，按錯的代價是「我明明想安靜，卻突然全部打開了」——正是這個功能要避免的事。要再打開就回去按上面那幾顆 |
| 兩種信**各自帶自己的 `kind`，不互相連坐** | 關掉櫻花樹不會連帶關掉逾期提醒，反過來也一樣。**連坐正是 Gmail 那顆按鈕犯的錯**，我們自己的出口不可以犯同一個。頁面上也明說「另一種不受影響」——沒說，使用者就會以為按下去是全部 |
| `kind` 是**白名單查表**，不是參數 | 查不到就退回。永遠不把使用者送來的字串拼進 SQL 的欄位名 |
| **GET 什麼都不做**，要按下按鈕才 POST | 郵件用戶端與公司的連結掃描器會自己去抓信裡的網址（同〈用過的重設連結保留 7 天〉踩過的坑）。讓 GET 就生效等於「信一進收件匣，提醒自己關掉了」 |
| token **存明碼**，與 `ics_feed` 只存雜湊不同 | 兩者的需求相反：ICS 的 token 給出去之後伺服器再也不需要它；退訂連結是**我們每寄一封信就要放一次**，雜湊回推不出來。代價講清楚：外洩時拿到它的人只能把某個人的通知**關掉**，登不了入、讀不到排程，而同一份外洩早就包含那個人的整份排程 |
| **只關不開** | 這個 token 打不開任何東西，要再打開一律回系統裡按。少了這一條，撿到連結的人就能把別人的提醒**打開**，那是騷擾 |
| **改到 0 列回 404，不回 ok** | 同〈破窗鎚〉的「改到 0 列當成錯誤」。回 ok 等於告訴他「關好了」而其實什麼都沒發生，下個月信照來——那是假綠燈，而且會讓他確信「這個連結沒用」，再回去按 Gmail 那顆 |
| 沒有 `APP_URL` 或沒有 token 就**整行不放** | 半個連結（`/unsub?t=` 後面空空的）比沒有連結更糟：點下去只會看到「連結不完整」，使用者會以為系統壞了 |
| **第一次要寄信時才產 token**，不在 migration 裡回填 | 回填要對每一列寫一次，而這個欄位只有「真的要寄信給他」時才用得到。也因此它放在所有 `continue` 之後——沒事、已經寄過、休假、停用的人都不該為此多一次寫入 |
| 併發時以**資料庫裡的那一個**為準 | `WHERE unsub_token IS NULL` 讓兩條路徑只有一條寫得進去。輸的那條要是回傳自己產的值，就會把一個從來不存在於資料庫裡的 token 寄出去——使用者點了之後改到 0 列，畫面上是「連結已失效」，而他什麼都沒做錯 |

**加一個新的公開頁面時，有三份寫死的清單要一起補**，漏掉的症狀都是「檢查是綠的，
但它根本沒看那個檔案」：

| 清單 | 在哪 | 漏掉會怎樣 |
|---|---|---|
| `FILES` | `tools/check-syntax.mjs` | 新頁面的 inline script 語法錯誤不會被抓到 |
| `WATCHED` | `tools/check-sw-version.mjs` | 改了新頁面卻沒加 `sw.js` 的版本號，使用者看到的還是舊的 |
| `assets` | `tools/smoke.mjs` 的 `makeServer` | 瀏覽器那一輪根本開不到那一頁 |

**假後端要能解析不帶副檔名的網址。** 信裡放的是 `/unsub`（線上由 Cloudflare 解析
成 `unsub.html`，`/reset` 就是這樣運作的）。假後端只認 `.html` 的話，測到的是一條
**信裡從來不會出現的網址**——那種綠燈比沒有測試更糟。第一次跑冒煙測試就是紅在
這裡，而紅的理由是工具不是產品。
...
**部署順序**：`migrations/005-app-purchase.sql`、`006-plan.sql`、`007-streak-mail.sql`、`008-push.sql`、`009-leave-days.sql`、`010-mail-log.sql` 、`011-mail-sender.sql` 與 `012-unsub-token.sql` 都要在部署新 Worker **之前**跑（`010` 是新增資料表，漏跑只會讓寄信多一行警告，不會 500；**`011` 是加欄位，漏跑會讓 `/api/admin/mail-log` 的 SELECT 直接失敗**）（`getSessionUser` 的 SELECT 讀 `plan_source`，欄位不在**每一個**登入請求都會 500；`007`／`008`／`009` 的欄位則是 `/api/reminder` 與 cron 會讀），理由見〈資料庫結構變更〉。006 跑完、Worker 部署完之後，還要到 `/admin` 把既有的兩個帳號設成「Pro（永久）」——漏掉的症狀是他們的第四個專案被擋，當場就會知道。
