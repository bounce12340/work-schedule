...
| archive 一律 `CODE_SIGN_IDENTITY="Apple Distribution"`（Release 設定與 workflow 兩處都寫） | Capacitor 範本預設 `iPhone Developer`，自動簽章因此去要**開發用**描述檔，那種必須綁至少一台實體裝置，帳號沒裝置就失敗（第一次打包就是這樣紅的：「Your team has no devices」）。上架要的是**發佈用**描述檔，不綁裝置；發佈憑證由 Xcode 雲端簽章代管，runner 不需要 .p12 |
