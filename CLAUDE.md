...
- 非循環項目：固定字串 `'single'`
- 每週／每兩週：`'W' + YYYY-MM-DD`（**未經假日調整**的原始日期）
- 每月循環：`'YYYY-MM'`
- 每季循環：`'YYYY-QN'`
...
2. 假日僅自動判斷週六日，國定假日需使用者自行加進「自訂假日」（支援一次貼上多個日期）。刻意不內建假日清單——日期每年由行政院公布且會因補班調整，寫死一份過期的清單會讓「遇假日順延」算出錯誤日期，比沒有這個功能更糟
3. 甘特圖長條為靜態百分比定位，不支援拖曳；日期只能透過表格輸入修改
4. 變更循環頻率會重置該項目的各次完成／覆寫／略過紀錄（見上方 occurrence 引擎章節）
5. 檢視狀態（目前頁籤、選取的年／季／月、搜尋與篩選條件）不存檔，重整回到預設；只有資料本身持久化。篩選條件尤其不該存——使用者下次打開會看到一份被篩選過的清單卻不知道為什麼有些項目不見了
6. 分享的資源不會併入自己的項目安排／日曆／指標，只出現在「共享」頁（理由見下方分享章節）
7. 「可操作」不含重新命名、改日期與刪除——那些只有擁有者能做
8. 分享過來的內容沒有本機快取，離線時看不到也改不了
