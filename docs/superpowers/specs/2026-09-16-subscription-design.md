...
| AI 小幫手 | **每天 3 次**（見假設 B） | 每天 20 次（現況） |

上限集中在一個物件，前端與 Worker 各一份**但形狀相同、有測試比對兩邊一致**：

```js
const PLAN_LIMITS = {
  free: { majorProjects: 3, ganttProjects: 3, aiPerDay: 3 },
...
**沒有上限**（假設 C）。沒有帳號就沒有東西可以訂，也沒有同步、分享、提醒、AI；那份檔案是展示品。把展示品鎖起來換不到任何錢。
...
寫這份文件時有三個地方你沒說，我先選了一邊。**任何一個不對都改得動**，只是要在 E1 動工前定下來：

| | 我選的 | 另一邊 |
|---|---|---|
| **A. 「專案」是哪一個** | **大項目與甘特專案各自最多 3 個**（兩種都算，各算各的） | 只算專案頁的甘特專案，大項目不限；或兩種加起來 3 個 |
| **B. AI 在免費版** | **每天 3 次**（每次呼叫要付 DeepSeek 錢，免費版不能不限；給 3 次是讓人試得到） | 免費版完全沒有 AI；或維持 20 次不分方案 |
| **C. 單機檔（雙擊開啟、沒登入）** | **沒有上限**（沒帳號就沒東西可訂；那是展示品） | 也套免費版的上限 |
