...
- **狀態**：**已結案**。根因是 SceneDelegate 建的是 `CAPBridgeViewController`，自寫的插件從未註冊；build 10 的診斷把答案指出來，第四輪改掉那一行並加上靜態接線檢查。**build 11 在真機上驗收通過**（見〈結案：build 11 的驗收〉）
...
修好之後**必須在真機上看到證據**，而且要指名看哪裡——這次追了四輪，每一輪都有「看起來像好了」的時刻。

驗收分兩半，因為壞掉的也是兩半：

| 驗什麼 | 畫面上的證據 | 它證明了 |
|---|---|---|
| **讀**得到 | 帳號頁「雲端同步**啟用中**」「已載入雲端版本」皆為綠色，**而且沒有 `signals=` 那一格** | 插件註冊成功、Keychain 讀得到 token、`/api/state` 通了。`#acctSignedOutDiag` 會在「走到單機模式而環境看起來像 app」時出現，它**不在**就是最強的證據 |
| **寫**得進去 | 把 app 完全關掉再打開，**不用重新登入** | `keychainSet` 真的落地了。這一半讀不出來——前一半全綠時它照樣可能是壞的（build 6～10 就是） |

**兩半要分開驗，因為它們壞掉的樣子完全不同。** 只看第一半的話，「登入後又被踢出來」那個無限輪迴會原封不動地留在那裡——當時帳號頁在被踢出去之前也是全綠的。

使用者的回報是「可以正常登入，然後也不會被踢出來」，兩半都過。

第 13 條帶走的東西：

13. **「修好了」只有真機能證明，而且要指名看哪裡。** 一個涵蓋八種成因的檢查套件擋得住回歸，證明不了這一次真的通了——CI 沒有 Mac，插件註冊只在真機上發生。驗收清單要像斷言一樣具體到「看哪一格、它該是什麼」，否則「看起來好像好了」會第五次騙過所有人。
