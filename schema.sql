...
CREATE TABLE IF NOT EXISTS reminder_feed (
  ...
  -- 退訂連結的 token：每一封訂閱信都帶著它，點下去就關掉**對應的那一個**開關。
  -- 沒有它的話，想停的人在 Gmail 裡只剩「取消訂閱」那顆按鈕可按，而那顆是說給
  -- 寄信商聽的——寄信商的退訂名單是帳號層級的，一按下去連密碼重設信都收不到
  -- （helen 2026-09-09 那次的真正成因，見 migrations/012）。
  --
  -- **存明碼，與 ics_feed 只存雜湊不同**：ICS 的 token 給出去之後伺服器再也不需要，
  -- 這個則是我們每寄一封信就要放一次，雜湊回推不出來。它能做的事只有「關掉通知」，
  -- 登不了入、讀不到資料、也打不開任何開關。
  --
  -- 可為 NULL：既有的列不必回填，第一次要寄信時才產生。
  unsub_token   TEXT,
  updated_at    INTEGER NOT NULL
);
