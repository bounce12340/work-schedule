...
- **遊戲化**：一株會長大的植物當角色。按時做完一件 +10 XP、那天安排的事全部按時做完（完美的一天）+30、拿到徽章 +100；連續天數、7→14→30 天的挑戰、徽章牆在「我的帳號」。沒安排的日子不算也不會斷。免費版與 Pro 都有
- **連續中斷的隔天早上，小植物會寄一封信**（可在「我的帳號」關掉）。只在斷掉的那一天寄、一天最多一封，連續不到兩天不寄
...
- **推播通知**（APNs）：逾期／即將到期、連續斷掉時植物的訊息、以及可選的「今天有 N 件事」。三個開關各自獨立，也與兩封信各自獨立；沒事的日子完全不推。三個 Worker secret：`APNS_KEY_ID`、`APNS_TEAM_ID`、`APNS_P8`
...
| 純告知 | `noticeOnly` 的項目只是一格佔位：沒有勾選框、**永遠不逾期**，四張指標卡、已完成區、範圍列的數字與小植物都不算它。行事曆訂閱看得到，提醒信與推播不通知 |
