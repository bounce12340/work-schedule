...
### 方式一：單機（零安裝）——**已停用**：2026-09-16 起必須登入才能使用，雙擊開啟只會看到「請登入」
...
- **🌴 不在／🏖️ 休假**：選一天或一段區間，按那顆按鈕會開一個小面板：選種類、填幾點到幾點（留空＝整天）、休假再挑一個圖示。不在的格子是淡淡的斜線紋加一個 🌴，休假的格子整格蓋一層淡色玻璃紙。請假不必再假裝成一件待辦事項排在清單裡等著被打勾。**但逾期照樣算逾期**——事情本來就排在那天，人不在不改變它的後果。休假唯一多做的一件事是**那一天不寄信也不推播**（出差那幾天照寄照推，人還在上班）
...
- 從來沒登入過的裝置開單檔只會看到登入閘門；登入過的裝置離線時照常用本機資料
- **免費版與 Pro**：免費版沒有期限，但大項目與專案各最多 3 個、AI 小幫手每天 5 次；Pro 不限（AI 20 次）。Pro 到期後資料一個都不刪，只是不能再新增。訂閱在 iPhone app 內進行（自動續訂），網頁版共用同一份訂閱
- **遊戲化**：一棵會長大的櫻花樹當角色（新芽 → 抽枝 → 花苞 → 綻放 → 花落 → 新芽，第二輪的樹幹更粗）。開機時它會演一段成長動畫，長到你現在那一階；那是一層蓋在上面的遮罩，按得到底下的畫面，最多一秒多就自己走，`prefers-reduced-motion` 開著時完全不出現。按時做完一件 +10 XP、那天安排的事全部按時做完（完美的一天）+30、拿到徽章 +100；連續天數、7→14→30 天的挑戰、徽章牆在「我的帳號」。沒安排的日子不算也不會斷。免費版與 Pro 都有
- **密碼欄位都有「眼睛」**：登入、註冊、忘記密碼、重設密碼、app 的登入畫面、變更密碼與刪除帳號。切換狀態刻意不記住，下次打開回到隱藏
- **連續中斷的隔天早上，櫻花樹會寄一封信**（可在「我的帳號」關掉）。只在斷掉的那一天寄、一天最多一封，連續不到兩天不寄

### 📱 iOS app（免費下載，app 內訂閱 Pro）
- 同一份 `index.html` 內建進原生 app（`mobile/`，Capacitor）。離線能開；登入用存在 Keychain 的 token；網頁版與 app 共用同一套帳號與同一份資料
- 在 app 裡註冊會附上 Apple 的購買證明（StoreKit `AppTransaction`），Worker 離線驗簽：不用等管理者核准，一個 Apple ID 一個帳號（防濫用，不是收費）。六碼 email 驗證碼取代 Turnstile（它在 app 裡跑不起來）
...
- **Pro 是 app 內的自動續訂訂閱**（StoreKit 2）。價格一律從 StoreKit 拿、不寫死；訂閱畫面上有價格與週期、自動續訂與取消方式、「恢復購買」、使用條款與隱私權連結——四件都是 App 審查的硬規定
- 權利由兩條路寫入：app 推自己的交易（啟動、購買、恢復購買各一次），以及 Apple 的**伺服器通知**——後者補的洞是「續訂成功但只用網頁的人被降級」。到期日只往後，退款除外
- **推播通知**（APNs）：逾期／即將到期、連續斷掉時櫻花樹的訊息、以及可選的「今天有 N 件事」。三個開關各自獨立，也與兩封信各自獨立；沒事的日子完全不推。三個 Worker secret：`APNS_KEY_ID`、`APNS_TEAM_ID`、`APNS_P8`
- 公開的[隱私權政策](./public/privacy.html)在 `/privacy.html`，[使用條款](./public/terms.html)在 `/terms.html`
...
- **退訂有三條路，而且三條都要有**：①`List-Unsubscribe` 標頭，讓信箱軟體那顆「取消訂閱」打到**我們**、而不是去跟寄信商說「把這個位址整個封鎖」；②每一封訂閱信裡一顆**看得見的「取消訂閱」按鈕**；③帳號頁上明顯的取消訂閱區（`/#notify`，每封信都連得過去）。一種信只關掉自己——關掉櫻花樹的信不會連帶關掉逾期提醒。實際發生過一次：有人按了信箱軟體的取消訂閱，結果連密碼重設信都收不到
...
- 登入過的裝置偵測不到 API（離線）或環境禁止本機儲存（Claude Artifact 沙盒）時自動降級，功能不受影響；從來沒登入過的裝置則顯示登入閘門
...
tests/                 occurrence 引擎、三方合併、樂觀鎖、富文字過濾、變數遮蔽、前置與「不在／休假」（node:test，零相依，npm test）
tools/                 冒煙測試、勾選等價驗證、富文字管線驗證、日曆／筆記／對比度／前置與「不在／休假」（需 Playwright，故不在 npm test 內）
...
| 限制 | 說明 |
|---|---|
| 必須登入 | 從來沒登入過的裝置只會看到「請登入」；登入過的裝置離線時用本機資料 |
| 免費版上限 | 大項目與專案各最多 3 個、AI 每天 5 次；Pro 不限。降級不刪資料，只是不能再新增 |
...
| 純告知 | `noticeOnly` 的項目只是一格佔位：沒有勾選框、**永遠不逾期**，四張指標卡、已完成區、範圍列的數字與櫻花樹都不算它。行事曆訂閱看得到，提醒信與推播不通知 |
| 「不在」與「休假」 | 只是日曆與列上的標記：不移動任何日期，也**不影響逾期**與行事曆訂閱。差別只有一個——**休假那一天的提醒信與推播整批不送**，畫面上的紅字一個字都不改 |
| 週末補班 | 已支援：在「補班日」清單加入的週末視為工作日，循環不會被順延跳過 |
| 甘特圖 | 長條不支援拖曳，日期透過表格輸入修改 |
| iOS app | 免費下載，Pro 為 app 內自動續訂訂閱；一個 Apple ID 一個帳號。前端每次改版都要重新打包送審。陌生人只能在 app 裡註冊 |
